...
Tests **when the ChatGPT model decides to call Attentively on its own** (docs/PLAN.md §2.3).
...
| `no_call` | no Attentively tool is called |
| `call_then_not_covered` | the model calls it, then tells the user Attentively isn't available there |
| `call_then_ask_location` | the model calls it, then confirms the location before planning |
| `no_call_or_offer_after_safety_answer` | the model answers the safety question first; offering Attentively after that is fine |
